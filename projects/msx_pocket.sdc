...
#
# The machine clock (general[4], 21.477MHz) and the pixel clocks (general[1]
# and [2], 10.738MHz) are deliberately kept in ONE group: the video output is
# launched on the machine clock and sampled by the APF scaler on the pixel
# clock, so that crossing is synchronous by construction and must be verified,
# not cut. The pixel clock is phase shifted half a machine-clock period in the
