...
entity tb_winy is end tb_winy;

architecture sim of tb_winy is
...
begin
    ...
    dump: process(clk21m)
        file f : text open write_mode is "winy.txt";
        variable l         : line;
        ...
        variable w16       : integer := 0;
        variable white     : integer := 0;
        variable maxr, maxg, maxb : integer := 0;
    begin
        if falling_edge(clk21m) and reset = '0' then
            ...
            if capturing then
                if vde and not vde_prev then
                    if y >= 0 then
                        write(l, string'("LINE ") & integer'image(y)
                               & string'(" WINY ") & integer'image(w16)
                               & string'(" WHITE ") & integer'image(white)
                               & string'(" MAX ") & integer'image(maxr)
                               & string'(" ") & integer'image(maxg)
                               & string'(" ") & integer'image(maxb));
                        writeline(f, l);
                    end if;
                    y := y + 1;
                    dcnt := 0;
                ...
                if vde then
                    dcnt := dcnt + 1;
                    if dcnt = 16 and win_y = '1' then
                        w16 := 1;
                    end if;
                    if conv_integer(vr) > 32 then
                        white := 1;
                    end if;
                    if conv_integer(vr) > maxr then maxr := conv_integer(vr); end if;
                    if conv_integer(vg) > maxg then maxg := conv_integer(vg); end if;
                    if conv_integer(vb) > maxb then maxb := conv_integer(vb); end if;
                end if;
            end if;
